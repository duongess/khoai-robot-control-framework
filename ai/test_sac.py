...
def test_gaussian_actor_returns_expected_tensor_shapes():
    actor = GaussianActor(state_dim=6, hidden_dim=32)
...
def test_gaussian_actor_actions_are_bounded():
    actor = GaussianActor(state_dim=3, hidden_dim=32)
...
def test_sac_agent_performs_one_training_update():
    agent = SACAgent(SACConfig(state_dim=3, hidden_dim=32, seed=7))
    batch = TensorBatch(
        states=torch.randn(8, 3),
        actions=torch.tanh(torch.randn(8, 1)),
